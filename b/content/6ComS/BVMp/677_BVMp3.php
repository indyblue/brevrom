<?php
	...
	hidden('Matins',2);
	if($_GET['matins']) require '677_BVMp3_matins.php';
	...
	if($long==0) {
		...
		space();
		ant('mirabile_mysterium_delcaratur_hodie.php','1');
	}
	space();	
	if($old==1) {
...
if($weekly) rubp('','Full Psalter: 
	...
	if($long==0) {
		...
		ant('prTemp/nativity/0101m2.php','1');
	}
	space();	
	if($old==1) {
		reading('vr/kyrie.php');
...
function bvm_multiant($part, $vespers=0, $option=1) {
	...
	ant('prTemp/nativity/0101L.php',$part);
