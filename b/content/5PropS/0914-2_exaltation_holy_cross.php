<?php img(); ?>
	<text:p text:style-name="BodySm"/>
<?php headSt('Sept. 14',2,'in Exaltatione Sanctæ Crucis','Exaltation of the Holy Cross'); ?>
<?php rubrics('head/1Vespers.php'); ?>
<?php rubrics('prSanct/Sun1Vespers.php'); ?>
<?php rubrics('prSanct/all2Vespers.php'); ?>
<?php rubrics('head/Magnificat.php'); ?>
<?php ant('prSanct/0914m1.php'); ?>
	<text:p text:style-name="BodySm"/>
<?php rubrics('head/Lauds.php'); ?>
<?php rubrics('head/Psalms.php'); ?>
<?php ant('prSanct/0914.php','20000'); ?>
<?php rubrics('ps/SuL1.php'); ?>
<?php ant('prSanct/0914.php','02222'); ?>
<?php rubrics('head/LittleChapter.php'); ?>
<?php lc('phil2_5.php'); ?>
<?php rubrics('asIn.php','lustra_sex_qui_jam_peregit','Ordinary of Passiontide','Hymn','Lustra sex qui jam perégit'); ?>
<?php vrS('prSanct/adoramus_te_christe_et_benedicimus_tibi.php'); ?>
<?php rubrics('head/Benedictus.php'); ?>
<?php ant('prSanct/0914b.php'); ?>
<?php rubrics('head/Prayer.php'); ?>
<?php prayer('prSanct/0914.php'); ?>
	<text:p text:style-name="BodySm"/>
<?php rubrics('head/LittleHours.php'); ?>
<?php rubrics('ps/Feria.php'); ?>
	<text:p text:style-name="BodySm"/>
<?php rubrics('head/Terce.php'); ?>
<?php rubrics('head/LittleChapter.php'); ?>
<?php lc('phil2_5.php'); ?>
<?php rubrics('head/BriefRespond.php'); ?>
<?php brS('prSanct/hoc_signum_crucis_erit_in_caelo.php'); ?>
<?php vrS('prSanct/adoramus_te_christe_et_benedicimus_tibi.php'); ?>
   <text:p text:style-name="BodySm"/>
<?php rubrics('head/Sext.php'); ?>
<?php rubrics('head/LittleChapter.php'); ?>
<?php lc('gal6_14.php'); ?>
<?php rubrics('head/BriefRespond.php'); ?>
<?php brS('prSanct/adoramus_te_christe_et_benedicimus_tibi.php'); ?>
<?php vrS('prSanct/omnis_terra_adoret_te_et_psallat_tibi.php'); ?>
   <text:p text:style-name="BodySm"/>
<?php rubrics('head/None.php'); ?>
<?php rubrics('head/LittleChapter.php'); ?>
<?php lc('phil2_8.php'); ?>
<?php rubrics('head/BriefRespond.php'); ?>
<?php brS('prSanct/omnis_terra_adoret_te_et_psallat_tibi.php'); ?>
<?php vrS('prSanct/hoc_signum_crucis_erit_in_caelo.php'); ?>
   <text:p text:style-name="BodySm"/>
<?php rubrics('head/2Vespers.php'); ?>
<?php rubrics('ps/antLauds.php'); ?>
<?php rubrics('ps/SuV116.php'); ?>
<?php rubrics('head/LittleChapter.php'); ?>
<?php lc('phil2_5.php'); ?>
<?php rubrics('asIn.php','vexilla_regis_prodeunt','Ordinary of Passiontide','Hymn','Vexílla Regis pródeunt'); ?>
<?php vrS('prSanct/hoc_signum_crucis_erit_in_caelo.php'); ?>
<?php rubrics('head/Magnificat.php'); ?>
<?php ant('prSanct/0914m2.php'); ?>
<?php rubrics('prSanct/ComplineSun.php'); ?>
